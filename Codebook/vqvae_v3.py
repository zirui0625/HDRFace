...
class SDVQVAE(nn.Module):
    def __init__(self, n_embed=2048, embed_dim=512, ch=256, out_ch=512, ch_mult=[1],
                 num_res_blocks=2, attn_resolutions=[16], dropout=0.0, in_channels=512,
                 resolution=64, z_channels=512, double_z=False, enable_mid=True,
        ...
        self.encoder2 = MultiHeadEncoder(ch=ch, out_ch=out_ch, ch_mult=ch_mult, num_res_blocks=num_res_blocks,
        ...
        self.decoder2 = MultiHeadDecoder(ch=ch, out_ch=out_ch, ch_mult=ch_mult, num_res_blocks=num_res_blocks,
                               attn_resolutions=attn_resolutions, dropout=dropout, in_channels=in_channels,
        ...
        self.idx_pred_layer = nn.Sequential(
            nn.LayerNorm(embed_dim),  # 对最后一个维度（即 512 维度）进行归一化。
            nn.Linear(embed_dim, n_embed, bias=False)  # 对归一化后的张量进行线性变换，将最后一个维度从 512 变为 2048。
        )
    ...
    def forward(self, x):
        ...
        hs = self.encoder2(latents)
        h = self.quant_conv(hs["out"])
        quant, emb_loss, info, dictionary = self.quantize(h)
        quant = self.post_quant_conv(quant)
        latents = self.decoder2(quant)
        x = self.decoder(latents).float().clamp(-1, 1)

        return x, emb_loss, info, hs, h, quant, dictionary
