...
class RMSNorm(torch.nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # [修改] 强制用 float32 计算 variance，对 variance 做 clamp 防止 rsqrt 输入为 0，
        #        避免 bfloat16 精度不足导致硬件级 SIGFPE
        hidden_states_f32 = hidden_states.to(torch.float32)
        variance = hidden_states_f32.square().mean(-1, keepdim=True)
        variance = variance.clamp(min=self.eps)  # [修改] 确保 rsqrt 输入严格大于 0
        hidden_states_f32 = hidden_states_f32 * torch.rsqrt(variance + self.eps)
        hidden_states = hidden_states_f32.to(input_dtype)
        if self.weight is not None:
            # [修改] weight 转为与 hidden_states 相同 dtype，避免混合精度乘法异常
...
class AdaLayerNorm(torch.nn.Module):
    def __init__(self, dim, single=False, dual=False):
        ...
        # [修改] eps 从 1e-6 改为 1e-5，给 bfloat16 精度留更多余量
